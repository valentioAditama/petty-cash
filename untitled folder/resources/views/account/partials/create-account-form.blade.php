<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __("Create new Account to use in the system.") }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('account.store') }}" class="mt-6 space-y-6">
        ...
        <div>
            <x-input-label for="debit" :value="__('Opening Balance')" />
            <x-text-input id="debit" name="debit" type="number"  step="any" min="0.00" class="mt-1 block w-full rounded-md "  :value="old('debit')" required />
