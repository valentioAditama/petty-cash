<section>
    ...
    <form method="post" action="{{ route('transaction.store') }}" class="mt-6 space-y-6">
        ...
        <div>
            <x-input-label for="account" :value="__('Nama')" />
        ...
        <div>
            ...
            <x-text-input id="date" name="date" type="date"
                class="mt-1 block w-full rounded-md dark:text-white " :value="old('date')" required />
            <x-input-error class="mt-2" :messages="$errors->get('date')" />
        </div>

        <div>
            <x-input-label for="amount" :value="__('Saldo')" />
            <div class="flex">
                <x-text-input id="amount" name="amount" type="number" step="any" min="0.00"
                    class="block min-w-0 w-full rounded-l-md flex-1 " :value="old('amount')" required />
                <span
        ...
        <div>
            <x-input-label for="message" :value="__('Catatan')" />
            <textarea id="message" name="note" rows="4"
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="Isi Catatan Disini...">{{ old('note') }}</textarea>
            <x-input-error class="mt-2" :messages="$errors->get('note')" />
        </div>

        <div>
            <x-input-label for="name" :value="__('Jenis Transaksi')" />
            <div class="flex items-center my-4">
                <input checked id="debit" type="radio" value="D" name="type"
                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                <label for="debit" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Pemasukan</label>

                <div class="p-4">
                    <input id="credit" type="radio" value="C" name="type"
                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    <label for="credit"
                        class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Pengeluaran</label>
                </div>
            </div>
        </div>
